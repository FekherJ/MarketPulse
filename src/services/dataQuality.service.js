...
function runDataQualityChecks(
  ...
  if (payload && Object.keys(payload).length > 0) {
    checks.push(
      createCheckResult(ingestionRunId, "PAYLOAD_NOT_EMPTY", "PASSED"),
    );
  } else {
    checks.push(
      createCheckResult(
  ...
  const missingCoins = expectedCoins.filter(
    (coin) => !payload || !payload[coin],
  );

  if (missingCoins.length === 0) {
    checks.push(
      createCheckResult(ingestionRunId, "EXPECTED_COINS_PRESENT", "PASSED"),
    );
  } else {
    checks.push(
      createCheckResult(
  ...
  const recordsWithoutSymbol = transformedRecords.filter(
